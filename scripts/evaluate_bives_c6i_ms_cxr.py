"""Run the authorized one-time local Qwen3.5-2B C6I MS-CXR evaluation."""
...
from bives_cxr.polarity_runtime import load_locked_polarity_checkpoint  # noqa: E402
...
FORMAT_VERSION = "bives_c6i_ms_cxr_evaluation_v1"
C6F_ROOT = ROOT / "local_runs/bives_cxr/c6_ms_cxr_postc5"
C6I_GEOMETRY_ROOT = ROOT / "local_runs/bives_cxr/c6i_ms_cxr_actual_input_geometry"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--geometry-lock", type=Path, default=C6I_GEOMETRY_ROOT / "c6i_geometry_lock.json")
    parser.add_argument("--geometry-rows", type=Path, default=C6I_GEOMETRY_ROOT / "c6i_geometry_rows.jsonl")
    parser.add_argument(
        "--geometry-certificates",
        type=Path,
        default=C6I_GEOMETRY_ROOT / "c6i_candidate_certificates.jsonl",
    )
    parser.add_argument("--geometry-mask-dir", type=Path, default=C6I_GEOMETRY_ROOT / "geometry_masks")
    parser.add_argument("--preopen-lock", type=Path, default=DEFAULT_ROOT / "c6i_preopen_lock.json")
...
def committed_code_identity() -> str:
    for command in (
        ...
        if subprocess.run(command, cwd=ROOT, check=False).returncode != 0:
            raise ValueError("C6I requires every tracked change committed before opening")
...
def main() -> None:
    ...
    lock = validate_c6i_preopen_lock(
        args.preopen_lock,
    ...
    if git_commit != lock["source_commit"]:
        raise ValueError("C6I committed source differs from pre-open lock")
    ...
    for path, expected in (
        ...
        if not path.is_file() or file_sha256(path) != expected:
            raise ValueError(f"C6I frozen artifact mismatch: {path}")
    if snapshot_model_files(model_path) != config["model"]["snapshot_sha256"]:
        raise ValueError("C6I Qwen3.5-2B snapshot changed")
    device = torch.device(args.device or config["model"]["device"])
    if device.type != "cuda" or not torch.cuda.is_available():
        raise ValueError("C6I requires an available local CUDA device")
    free_bytes, _ = torch.cuda.mem_get_info(device)
    if free_bytes < 20 * 1024**3:
        raise ValueError(f"C6I requires at least 20 GiB free on {device}")
    ...
    marker_path = args.output_dir / "C6I_EVALUATION_OPENED.json"
    metrics_path = args.output_dir / "metrics_final.json"
    progress_path = args.output_dir / "progress.json"
    if metrics_path.exists():
        raise ValueError("C6I is already complete and cannot be rerun")
    if marker_path.exists():
        if json.loads(marker_path.read_text(encoding="utf-8")) != identity:
            raise ValueError("existing C6I opening marker identity differs")
    else:
    ...
    if progress_path.exists():
        ...
        if progress.get("identity") != identity:
            raise ValueError("existing C6I progress identity differs")
    ...
    if (
        ...
        raise ValueError("C6I checkpoint semantic identity changed")
    ...
    for index, row in enumerate(rows, start=1):
        ...
        if file_sha256(image_path) != row["official_image_sha256"]:
            raise ValueError(f"C6I image hash mismatch: {sample_id}")
        with Image.open(image_path) as handle:
            image = handle.convert("RGB")
        geometry_row = geometry[sample_id]
        actual_size = (int(geometry_row["actual_columns"]), int(geometry_row["actual_rows"]))
        if image.size != EXPECTED_ACTUAL_SIZE or image.size != actual_size:
            raise ValueError(f"C6I actual JPG geometry changed: {sample_id}")
        mask_path = args.geometry_mask_dir / str(geometry_row["mask_file"])
        if file_sha256(mask_path) != geometry_row["mask_sha256"]:
            raise ValueError(f"C6I geometry mask changed: {sample_id}")
        ...
        if letterboxed.size != (IMAGE_SIZE, IMAGE_SIZE):
            raise ValueError(f"C6I Qwen input geometry changed: {sample_id}")
        ...
        random_mask = deterministic_random_mask(
            ...
            content,
            seed_key=f"{sample_id}::c6i-localization::{config['evaluation']['bootstrap_seed']}",
        )
        ...
        completed[sample_id] = {
            ...
            "positive_only": True,
            "coordinate_transform_version": geometry_row["coordinate_transform_version"],
            "actual_image_size": list(image.size),
        ...
        print(f"C6I_SCORE {index}/{len(rows)}", flush=True)

    final_rows = [completed[key] for key in sorted(completed)]
    if len(final_rows) != 29:
        raise ValueError("C6I result table is incomplete")
    operator_results = {
    ...
    result = {
        ...
        "route_decision": "terminal_stop_after_c6i_pass_or_fail",
