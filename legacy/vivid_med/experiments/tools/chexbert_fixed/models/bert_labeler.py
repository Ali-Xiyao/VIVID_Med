...
# PATCH (offline fix): resolve the base BERT from a local directory instead of the
# HF hub by name. transformers 5.x cannot resolve 'bert-base-uncased' offline, and
# refuses to load the cached pytorch_model.bin on torch<2.6. Pointing this at a local
# dir that contains model.safetensors lets the model build fully offline. Model
# architecture/forward logic is unchanged.
_BERT_BASE_UNCASED = os.environ.get("CHEXBERT_BERT_PATH", "bert-base-uncased")
...
class bert_labeler(nn.Module):
    def __init__(self, p=0.1, clinical=False, freeze_embeddings=False, pretrain_path=None):
        ...
        if pretrain_path is not None:
            ...
            self.bert = BertModel.from_pretrained(_BERT_BASE_UNCASED)
