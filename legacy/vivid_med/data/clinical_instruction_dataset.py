...
def normalize_vsl_label(value: Any) -> str:
    raw = str(value or "").strip().lower()
    if raw in VSL_LABELS:
        return raw
    if raw in {"unsupported", "not_supported", "no_evidence"}:
        return "insufficient"
    return ""
...
class Qwen3VLInstructionCollator:
    ...
    def _single_token_id_for_value(self, value: str) -> int | None:
        for sequence in self._token_sequences_for_value(value):
            if len(sequence) == 1:
                return int(sequence[0])
        return None

    def _canonical_vsl_label(self, item: dict[str, Any]) -> str:
        metadata = item.get("metadata") or {}
        for key in ("sufficiency_label", "answer", "answer_short", "state"):
            label = normalize_vsl_label(metadata.get(key) if key in metadata else item.get(key))
            if label:
                return label
        return normalize_vsl_label(item.get("answer"))
    ...
    def __call__(self, batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        inputs["vsl_labels"] = [self._canonical_vsl_label(item) for item in batch]
        label_token_ids = [self._single_token_id_for_value(label) for label in VSL_LABELS]
        if all(token_id is not None for token_id in label_token_ids):
            inputs["vsl_label_token_ids"] = torch.tensor(label_token_ids, dtype=torch.long)
