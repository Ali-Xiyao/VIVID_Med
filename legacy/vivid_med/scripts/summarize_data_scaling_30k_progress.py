"""Summarize completed 30k data-scaling rows."""
...
RUNS = [
    {
        ...
        "sample_size": "29000",
        "run_dir": ROOT / "outputs" / "data_scaling" / "bce_30k",
        "claim_boundary": "source_control_only_30k_named_split",
    },
    {
        ...
        "stage": "source_classifier",
        "sample_size": "29000",
        "run_dir": ROOT / "outputs" / "data_scaling" / "no_lm_ums_30k",
        "claim_boundary": "source_checkpoint_for_lp_30k_named_split",
    },
    {
        ...
        "stage": "linear_probe",
        "sample_size": "29000",
        "run_dir": ROOT / "outputs" / "data_scaling" / "lp_no_lm_ums_30k",
        "claim_boundary": "linear_probe_from_no_lm_source_30k_named_split",
    },
    {
        ...
        "stage": "linear_probe",
        "sample_size": "29000",
        "run_dir": ROOT / "outputs" / "data_scaling" / "lp_frozen_lm_ums_30k",
        "claim_boundary": "linear_probe_from_frozen_lm_source_30k_named_split",
    },
]
...
def main() -> None:
    ...
    write_csv(FINAL_DIR / "data_scaling_30k_progress.csv", key_rows, columns)
    write_csv(FINAL_DIR / "data_scaling_30k_trajectory.csv", trajectory, trajectory_columns)

    text = "# Data Scaling 30k Progress\n\n"
    text += (
        "This table summarizes completed 30k data-scaling rows. "
        "At this gate the BCE source-control row, no-LM UMS source+LP branch, and any completed frozen-LM LP row are summarized. "
        "Frozen-LM source-loss trajectory is summarized separately in `data_scaling_frozen_source_progress.*`. "
        "The named 30k split currently contains 29000 train records.\n\n"
    )
    text += "## Key Rows\n\n"
    ...
    text += "\n## Claim Boundary\n\n"
    text += (
        "- `bce_30k` is a source-control row only.\n"
        "- `no_lm_ums_30k` is a source row and creates the checkpoint required by `lp_no_lm_ums_30k`.\n"
        "- `lp_no_lm_ums_30k` is the completed no-LM linear-probe row for the 30k named split.\n"
        "- `lp_frozen_lm_ums_30k`, when present, completes the frozen-LM linear-probe row from the completed frozen-LM source checkpoint.\n"
        "- Report actual train records as 29000 even though the split/config name is 30k.\n"
        "- Preserve final, best-val, best-AUC, and best-F1 policies separately.\n"
    )
    (FINAL_DIR / "data_scaling_30k_progress.md").write_text(text, encoding="utf-8")
    print(f"Wrote 30k data-scaling summaries to {rel(FINAL_DIR)}")
