...
def compute_hierarchical_vsl_loss(
    logits: torch.Tensor,
    labels: torch.Tensor,
    batch: dict[str, Any],
    config: dict[str, Any],
) -> tuple[torch.Tensor, dict[str, float]]:
    cfg = (config.get("training", {}) or {}).get("hierarchical_vsl_loss", {}) or {}
    if not bool(cfg.get("enabled", False)):
        return logits.new_zeros(()), {}

    label_token_ids = batch.get("vsl_label_token_ids")
    if label_token_ids is None:
        raise ValueError("hierarchical_vsl_loss requires single-token ids for support/contradict/uncertain/insufficient")
    label_token_ids = label_token_ids.to(device=logits.device, dtype=torch.long)
    if int(label_token_ids.numel()) != 4:
        raise ValueError("hierarchical_vsl_loss expected four VSL label token ids")

    label_to_index = {"support": 0, "contradict": 1, "uncertain": 2, "insufficient": 3}
    raw_labels = batch.get("vsl_labels") or []
    target_indices: list[int] = []
    row_indices: list[int] = []
    token_positions: list[int] = []
    for row_idx, raw_label in enumerate(raw_labels):
        target_index = label_to_index.get(str(raw_label))
        if target_index is None:
            continue
        active_positions = torch.nonzero(labels[row_idx] != -100, as_tuple=False).flatten()
        if active_positions.numel() == 0:
            continue
        pos = int(active_positions[0].item())
        if pos <= 0:
            continue
    ...
    pos_tensor = torch.tensor(token_positions, dtype=torch.long, device=logits.device)
    targets = torch.tensor(target_indices, dtype=torch.long, device=logits.device)
    class_logits = logits[row_tensor, pos_tensor][:, label_token_ids].float()

    answerable_target = (targets <= 1).float()
    answerable_logits = torch.stack(
        [
            torch.logsumexp(class_logits[:, :2], dim=1),
            torch.logsumexp(class_logits[:, 2:], dim=1),
        ],
        dim=1,
    )
    answerable_loss = F.cross_entropy(answerable_logits, answerable_target.long())

    support_contradict_mask = targets <= 1
    if bool(support_contradict_mask.any()):
        support_contradict_loss = F.cross_entropy(
            class_logits[support_contradict_mask, :2],
            targets[support_contradict_mask],
        )
    else:
        support_contradict_loss = logits.new_zeros(())

    uncertain_target = (targets == 2).float()
    non_uncertain_logit = torch.logsumexp(class_logits[:, [0, 1, 3]], dim=1)
    uncertain_logit = class_logits[:, 2] - non_uncertain_logit
    uncertain_loss = F.binary_cross_entropy_with_logits(uncertain_logit, uncertain_target)

    answerable_weight = float(cfg.get("answerable_weight", 1.0))
    support_contradict_weight = float(cfg.get("support_contradict_weight", 1.0))
    uncertain_weight = float(cfg.get("uncertain_weight", 1.0))
    total = (
        answerable_weight * answerable_loss
        + support_contradict_weight * support_contradict_loss
        + uncertain_weight * uncertain_loss
    )
    metrics = {
        "hier_answerable_loss": float(answerable_loss.detach().cpu()),
        "hier_support_contradict_loss": float(support_contradict_loss.detach().cpu()),
        "hier_uncertain_loss": float(uncertain_loss.detach().cpu()),
        "hierarchical_vsl_loss": float(total.detach().cpu()),
    }
...
def compute_training_loss(model: torch.nn.Module, batch: dict[str, Any], config: dict[str, Any]) -> tuple[torch.Tensor, dict[str, float]]:
    ...
    total = ce_loss
    hierarchy_loss, hierarchy_metrics = compute_hierarchical_vsl_loss(outputs.logits, batch["labels"], batch, config)
    if hierarchy_metrics:
        hierarchy_weight = float(((config.get("training", {}) or {}).get("hierarchical_vsl_loss", {}) or {}).get("weight", 1.0))
