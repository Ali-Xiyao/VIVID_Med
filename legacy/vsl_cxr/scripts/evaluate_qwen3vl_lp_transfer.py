...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--model-path", help="Override model.model_path from the LP config.")
    parser.add_argument("--vision-checkpoint", help="Override model.vision_checkpoint from the LP config.")
    ...
    parser.add_argument(
        "--primary-labels",
        nargs="+",
        help="Optional label subset for the primary external metrics; full mapped-label metrics are still retained.",
    )
...
def evaluate_collect(
    ...
    primary_label_names: list[str] | None = None,
    ...
    if primary_label_names:
        missing = [label for label in primary_label_names if label not in label_names]
        if missing:
            raise ValueError(f"Primary labels are absent from the probe head: {missing}")
        indices = [label_names.index(label) for label in primary_label_names]
        primary_true = y_true[:, indices]
        primary_prob = y_prob[:, indices]
        primary_pred = (primary_prob >= 0.5).astype(int)
        result["primary_label_names"] = primary_label_names
        result["primary_metrics"] = compute_classification_metrics(
            y_true=primary_true,
            y_pred=primary_pred,
            y_prob=primary_prob,
            label_names=primary_label_names,
            threshold=0.5,
        )
    return result
...
def main() -> None:
    ...
    if args.vision_checkpoint:
        config.setdefault("model", {})["vision_checkpoint"] = args.vision_checkpoint
    if args.device:
    ...
    result = evaluate_collect(
        ...
        label_names=label_names,
        primary_label_names=args.primary_labels,
    ...
    metrics = {
        ...
        "primary_label_names": result.get("primary_label_names", []),
