...
5. 但外部泛化证据必须保守书写：NIH 只能作为 appendix/stress；主 external 仍因 VinBigData/PadChest/MIMIC 标签或 manifest 边界阻塞。
...
1. 主 external 未完成：VinBigData-derived 本地只有 PNG 和 `train_meta.csv`，没有 class label/bbox CSV；PadChest 缺失；MIMIC-CXR 缺 audited CheXpert label manifest。
...
| Dataset | 状态 | 说明 |
|---|---|---|
| VinDr-CXR / VinBigData | blocked_missing_class_labels | 本地有 PNG 和 `train_meta.csv`，但没有 class label/bbox CSV |
| PadChest | missing | 本地未找到 |
| MIMIC-CXR | exists_missing_label_manifest | 本地有数据目录，但缺 audited CheXpert label manifest |
...
1. 主 external：
   - 找到 VinBigData/VinDr labels/bbox CSV；
   - 或整理 MIMIC-CXR CheXpert label manifest；
   - 或加入可靠 PadChest mapping。
2. 多 seed：
