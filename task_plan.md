...
| C6C MS-CXR official-test intake tooling | complete_structure_preflight_license_pending | The v1.1.0 ZIP is under the public-data root and all publisher checksums pass. Pair grouping now follows unique `(image_id, category_id, label_text)` keys while validating all 25/20 component boxes. The real preflight binds and hashes all 29 images, reports zero patient/study overlap, and records `license_gate_passed=false` plus `model_evaluation_authorized=false`. Strict intake still needs a user-authored access attestation; post-C5 model authority remains a separate blocked gate. |
