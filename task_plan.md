...
| VT4 build new V0 score-free development lock | complete | The 280-image globally disjoint VinDr-train data lock excludes all 1,446 ARISE identities. Geometry v2 completed 32/32 rows with 30 translated controls and 2 score-blind connected fallbacks; no score entered geometry selection. |
| VT5 implement and run V0 dose-response | complete_terminal_fail | Qwen cache 280/280; independent head minima 0.92/0.75; 384/384 rows complete. Only 4/12 cells pass and zero families survive all findings. Corrected result canonical SHA-256 `3c9ceb27...30bb1`; case-study SHA-256 `48b9c78e...24a93`. |
| VT6 run V1 coverage-redundancy only if V0 passes | locked_by_v0_failure | V0 did not pass. Do not run coverage-redundancy, coalition selector, U/I, scaling, or test. |
