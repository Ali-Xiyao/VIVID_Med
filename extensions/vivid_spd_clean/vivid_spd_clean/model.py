"""Exact paired projectors and the frozen-Qwen token-distillation model."""
...
class HistoricalPrefixProjector(nn.Module):
    """Historical A+UMS projector: four prefixes plus projected ViT tokens."""
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__()
        if prefix_tokens not in {4, 8}:
            raise ValueError("prefix projector supports primary 4 or diagnostic 8")
        self.prefix_tokens = nn.Parameter(
            torch.randn(1, prefix_tokens, output_dim) * 0.02
        )
    ...
    def orthogonality_loss(self) -> torch.Tensor:
        return self.prefix_tokens.sum() * 0.0


class HistoricalSPDProjector(nn.Module):
    """Historical four-by-two SPD with projected ViT tokens."""
    ...
    def __init__(
        ...
        *,
        ...
        if num_groups != 4 or tokens_per_group != 2:
            raise ValueError("strict SPD is frozen at four groups by two tokens")
...
def build_projector(
    ...
    *,
    ...
    if arm == "ums_prefix4":
        return HistoricalPrefixProjector(
    ...
    if arm in {"ums_spd4x2", "ums_spd4x2_no_ortho"}:
        return HistoricalSPDProjector(
            vision_dim=vision_dim,
            output_dim=output_dim,
        )
...
class VividSPDTokenModel(nn.Module):
    """Trainable ViT/projector feeding a frozen local Qwen3.5 teacher."""
    ...
    @property
    def orthogonality_weight(self) -> float:
        return 0.02 if self.arm == "ums_spd4x2" else 0.0
