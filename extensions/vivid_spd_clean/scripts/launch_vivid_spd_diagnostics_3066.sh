...
STRICT_ROOT="$ROOT/local_runs/strict_vivid_spd_qwen35_2b_20260723_s0_s3"
RUN_ROOT="$ROOT/local_runs/strict_vivid_spd_qwen35_2b_20260724_diagnostics"
...
echo "$(date -Is) allocation-3066 step queue is clear; launching diagnostics"
...
exec srun \
  ...
  --cpus-per-task=4 \
  --gres=gpu:1 \
  "$PY" scripts/run_vivid_spd_bounded_diagnostics.py \
    --run-root "$RUN_ROOT" \
    ...
    --strict-lock "$ROOT/audit/vivid_spd_clean_lock.json" \
    --diagnostic-lock "$ROOT/audit/vivid_spd_bounded_diagnostic_lock.json" \
    --device cuda:0
