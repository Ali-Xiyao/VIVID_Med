...
class SPDHFPProjector(nn.Module):
    """
    V12: SPD + HFP 联合投影器
    HFP 多层融合（纵向多尺度）→ SPD cross-attention 分解（横向多方面）

    流程：
    1. HFP: 收集 ViT 多层特征 → per-layer MLP → concat → fusion → fused_features
    2. SPD: 多组 query tokens cross-attend to fused_features → SPD tokens
    3. 共享 MLP 投影到 LLM 空间
    """
    ...
    def __init__(
        ...
        self.num_groups = num_groups
        self.tokens_per_group = tokens_per_group
        self.num_hfp_layers = num_hfp_layers
        if mlp_hidden_dim is None:
            mlp_hidden_dim = vit_embed_dim * 2

        # --- HFP 部分：per-layer projectors + fusion ---
        hfp_hidden = vit_embed_dim * 2
        self.layer_projectors = nn.ModuleList([
            nn.Sequential(
                nn.Linear(vit_embed_dim, hfp_hidden),
                nn.GELU(),
                nn.Dropout(dropout),
                nn.Linear(hfp_hidden, vit_embed_dim),
            )
            for _ in range(num_hfp_layers)
        ])
        self.fusion = nn.Sequential(
            nn.Linear(vit_embed_dim * num_hfp_layers, vit_embed_dim),
            nn.GELU(),
            nn.Dropout(dropout),
        )
        self.fusion_norm = nn.LayerNorm(vit_embed_dim)

        # --- SPD 部分：cross-attention query tokens ---
        self.group_queries = nn.ParameterList([
            nn.Parameter(torch.randn(1, tokens_per_group, vit_embed_dim) * 0.02)
            for _ in range(num_groups)
        ])
    ...
    def forward(
        ...
        hfp_features: Optional[dict] = None,
    ) -> torch.Tensor:
        """
        Args:
            vit_features: (B, N, vit_embed_dim) ViT 最后一层输出
        ...
        # 1. HFP 多层融合
        if hfp_features and len(hfp_features) >= self.num_hfp_layers:
            sorted_keys = sorted(hfp_features.keys())[:self.num_hfp_layers]
            layer_feats = [hfp_features[k] for k in sorted_keys]
            projected = []
            for i, feat in enumerate(layer_feats):
                if feat.dim() == 2:
                    feat = feat.unsqueeze(1)
                projected.append(self.layer_projectors[i](feat))
            concat = torch.cat(projected, dim=-1)  # (B, N, vit_dim * num_layers)
            fused = self.fusion(concat)  # (B, N, vit_dim)
            fused = self.fusion_norm(fused)
        else:
            fused = vit_features  # fallback: 纯 SPD 模式

        # 2. SPD cross-attention on fused features
        self._attn_weights = []
        group_outputs = []
        for i in range(self.num_groups):
            queries = self.group_queries[i].expand(B, -1, -1)
        ...
        # 3. 投影到 LLM 空间
        spd_projected = self.layer_norm(self.mlp(spd_tokens))
        vit_projected = self.layer_norm(self.mlp(fused))
